...
public class StockService {
    ...
    private final int restockThreshold;  // Make the restock threshold configurable

    // Constructor allows flexibility in setting the threshold
    public StockService(IStockDAO stockDAO, IShelfStockDAO shelfStockDAO, IBatchDAO batchDAO, int restockThreshold) {
        this.stockDAO = stockDAO;
        this.shelfStockDAO = shelfStockDAO;
        this.batchDAO = batchDAO;
        this.restockThreshold = restockThreshold;  // Initialize the threshold dynamically
    }

    // Method to check if restocking is needed and perform restocking if required
    public void checkAndRestockShelf(int productId) throws SQLException {
        int currentShelfStock = shelfStockDAO.getShelfStockForProduct(productId);
        if (currentShelfStock < restockThreshold) {  // Use the configurable threshold
            restockShelfFromStock(productId, restockThreshold - currentShelfStock);
        }
    }
